...
function dgDatagrid(){
	...
	$.ajax({  
        type : "post",  
        async : false,
        url : "datastatistics/getRobotFauit"+chartStr,
        data : {},  
            		 ...
            		 if(i==result.ary.length-1){
                		 column.push({field:"t"+i,title:result.ary[i].title,width:100,halign : "center",align : "left",sortable : true,
             				sorter : function(a,b){
             					...
             					return "<a href='datastatistics/goRobotFauitDetail?id="+row.t0+"&time1="+dtoTime1+"&time2="+dtoTime2+"&fauit="+row.t4+"'>"+value+"</>";
             				}});
            		 }else{
            			 column.push({field:"t"+i,title:result.ary[i].title,width:100,halign : "center",align : "left",sortable : true,
            				sorter : function(a,b){
...
function fauitcombobox(){
	$.ajax({  
      type : "post",  
      async : false,
      url : "datastatistics/getAllRobotFauit", 
      data : {},  
      dataType : "json", //返回数据形式为json  
      success : function(result) {  
          if (result) {
