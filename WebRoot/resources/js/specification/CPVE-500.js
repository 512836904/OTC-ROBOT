...
/*	if ($('#frequency').numberbox('getValue') < (0.5) || $('#frequency').numberbox('getValue') > (32)) {
	...
	if(machineModel==171){
		...
		if ($('#fweld_tuny_ele').numberbox('getValue') < (0) || $('#fweld_tuny_ele').numberbox('getValue') > (255)) {
			alert("焊接电流微调范围：0~255");
			return false;
		}
	}
	...
	if ($('#fselect').combobox('getValue') == 102) {
		if(machineModel==171){
			...
			if ($('#fweld_tuny_vol').numberbox('getValue') < (0) || $('#fweld_tuny_vol').numberbox('getValue') > (255)) {
				alert("焊接电压微调范围：0~255");
				return false;
			}
		}
...
//下发参数时对页面已写好的参数进行检测
function cpvewValidationFrom(){
	return $("#fm").form('enableValidation').form('validate');
}
function cpvewSendCheck() {
	if (!cpvewValidationFrom()) {
		return;
	}
	if (CPVEWCHECK() == false) {
		return;
