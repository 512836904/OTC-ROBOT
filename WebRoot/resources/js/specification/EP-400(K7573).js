...
/*
	...
	var xiafasend1 = mach + chanel + ftime + fadvance + fini_ele + fini_vol + fini_vol1 + fweld_ele + fweld_vol + fweld_vol1 + farc_ele + farc_vol + farc_vol1 + fhysteresis + fcharacter + fgas
		+ fdiameter + fmaterial + fweldprocess + con + fweld_tuny_ele + fweld_tuny_vol + farc_tuny_ele + farc_tuny_vol  + "0000000000000000";
...
//下发参数时对页面已写好的参数进行检测
function epsValidationFrom(){
	return $("#fm").form('enableValidation').form('validate');
}
function epsSendCheck() {
	if (!epsValidationFrom()) {
		return;
	}
	if (EPSCHECK() == false) {
		return;
...
//打开通道复制的dialog
function openEpsCopyDialog(value){
	WeldInsframework();
	var url="";
	if (value == 1 || value == 3) {
				...
				if (result) {
					...
					for (var q = 0; q < rows.length; q++) {
						...
						for (var n = 0; n < yshu1.length; n++) {
							...
							allIssueRows.push(EPS(yshu1[n], rows[q].gatherId));
						}
					}
				} else {
