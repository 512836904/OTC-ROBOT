...
//下发参数时对页面已写好的参数进行检测
function wbpValidationFrom(){
	return $("#fm").form('enableValidation').form('validate');
}
function wbpSendCheck() {
	if (!wbpValidationFrom()) {
		return;
	}
	if (WBPCHECK() == false) {
		return;
...
//打开通道复制的dialog
function openWbpCopyDialog(value){
	WeldInsframework();
	var url="";
	if (value == 1 || value == 3) {
