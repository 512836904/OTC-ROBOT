...
function WBW(yshu,gather){
	...
	if (fgas == parseInt(123).toString(16)) {
		fgas = "4";
	} else if (fgas == parseInt(121).toString(16)) {
		fgas = "2";
...
//下发参数时对页面已写好的参数进行检测
function wbwValidationFrom(){
	return $("#fm").form('enableValidation').form('validate');
}
function wbwSendCheck() {
	if (!wbwValidationFrom()) {
		return;
	}
	if (WBWCHECK() == false) {
		return;
...
//打开通道复制的dialog
function openWbwCopyDialog(value){
	WeldInsframework();
	var url="";
	if (value == 1 || value == 3) {
				...
				if (result) {
					...
					for (var q = 0; q < rows.length; q++) {
						...
						for (var n = 0; n < yshu1.length; n++) {
							...
							allIssueRows.push(WBW(yshu1[n], rows[q].gatherId));
						}
					}
				} else {
