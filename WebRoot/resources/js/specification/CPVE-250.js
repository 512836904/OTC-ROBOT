...
//下发参数时对页面已写好的参数进行检测
function cpvetValidationFrom(){
	return $("#fm").form('enableValidation').form('validate');
}
function cpvetSendCheck() {
	if (!cpvetValidationFrom()) {
		return;
	}
	if (CPVETCHECK() == false) {
		return;
...
//打开通道复制的dialog
function openCpvetCopyDialog(value){
	WeldInsframework();
	var url="";
	if (value == 1 || value == 3) {
				...
				if (result) {
					...
					for (var q = 0; q < rows.length; q++) {
						...
						for (var n = 0; n < yshu1.length; n++) {
							...
							allIssueRows.push(CPVET(yshu1[n], rows[q].gatherId));
						}
					}
				} else {
