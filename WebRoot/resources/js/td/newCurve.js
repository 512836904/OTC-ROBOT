...
//				showChart();
				...
				for(var i=0;i<machine.length;i++){
					...
					var ftype = machine[i].ftype;
					var imgnum = machine[i].model;
					if(ftype==45){
						var str = '<div id="machine'+machine[i].fid+'" style="width:500px;height:120px;float:left;margin-right:10px;display:none">'+
						'<fieldset style="border-width: 2px;float:left;border-radius:8px;margin-top:1px; border-color: #008000; width:100%; height:100%">'+
						'<div style="float:left;width:20%;height:100%;"><a href="td/goNextcurves?value='+machine[i].fid+'&valuename='+machine[i].fequipment_no+'&type='+machine[i].model+'&form='+ftype+'"><img id="img'+machine[i].fid+'" src="resources/images/welder_'+imgnum+'3.png" style="height:110px;width:90%;padding-top:10px;"></a></div>'+
						'<div style="float:left;margin-top: 2px;width:5px;height: 100%;background-color: #66CCCC;"></div>'+
						...
						'<li style="width:100%;height:19px;">焊机状态：<span id="m6'+machine[i].fid+'">关机</span></li></ul><input id="status'+machine[i].fid+'" type="hidden" value="3"></div>'+
						'<div style="float:left;margin-top: 2px;width:5px;height: 100%;background-color: #66CCCC;"></div>'+
						'<div style="float:left;width:20%;height:100%;"><img id="img'+machine[i].fid+'" src="resources/images/welder_'+0+'3.png" style="height:110px;width:90%;padding-top:10px;"></div>'+
						'<div style="float:left;width:25%;height:100%;">'+
						'<ul><li style="width:100%;height:19px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis">焊机编号：<span id="m1'+machine[i].fid+'">'+machine[i].weldmachine+'</span></li></ul></fieldset></div>';
					$("#curve").append(str);
	...
	if(redata.length==297 || redata.length%99==0){
		for(var i = 0;i < redata.length;i+=99){
						...
						if(liveary.length==0){
							...
							for(var x=0;x<liveary.length;x++){
								if(liveary[x].fid == machine[f].fid){
									tempflag = true;
									liveary.splice(x, 1, {"fid":machine[f].fid,
										"liveele":liveele+"A",
										"livevol":livevol+"V",
										"livestatus":livestatus,
										"livestatusid":livestatusid,
			...
			for(var i = 0;i < redata.length;i+=73){
				//循环遍历，找到与接收到数据的设备id相匹配的设备，在页面上显示
				for(var f=0;f<machine.length;f++){
					if(machine[f].fid==(parseInt(redata.substring(0+i, 4+i),10))){
							var imgnum = machine[f].model;
							$("#m3"+machine[f].fid).html("--");
							$("#m2"+machine[f].fid).html("--");
//							for(var k=0;k<welderName.length;k++){
...
//							}
							if(parseInt(redata.substring(32+i, 36+i),10)==137){
								...
								var liveele = parseInt(redata.substring(6+i, 10+i),10);
							}
				            var livevol = parseFloat(parseInt(redata.substring(10+i, 14+i),10)/10).toFixed(1);
//				            var maxele = parseInt(redata.substring(75+i, 79+i),10);
//				            var minele = parseInt(redata.substring(79+i, 83+i),10);
//				            var maxvol = parseInt(redata.substring(83+i, 87+i),10);
//				            var minvol = parseInt(redata.substring(87+i, 91+i),10);
							...
							case "03":
								livestatus = "工作";
								livestatusid = 0;
								liveimg = "resources/images/welder_"+imgnum+"3.png";
								break;
							case "04":
								livestatus = "电流过低";
								livestatusid = 2;
